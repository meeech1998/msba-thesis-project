...
def style_bar_chart(
    ...
    colors = [
        HIGHLIGHT_COLOR if cat in highlight_labels else NEUTRAL_COLOR
...
def style_volume_line_chart(
    ...
    plt.figure(figsize=figsize)
    plt.plot(dates, values, linewidth=2, color=HIGHLIGHT_COLOR)
    ...
    if not series.empty:
        ...
        plt.scatter(peak_date, peak_val, color=HIGHLIGHT_COLOR, s=35, zorder=3)
        plt.annotate(
...
def style_percent_line_chart(
    ...
    plt.figure(figsize=figsize)
    plt.plot(dates, values, linewidth=1.8, marker="o", markersize=3.5, color=HIGHLIGHT_COLOR)
